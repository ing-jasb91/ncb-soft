...
# Bucle "for in" para iterar el respaldo de todo los dispositivos en la tabla "devices"
...
for data in listDevices :
    ...
    if path.exists(localFilePath):
        ...
        try:
            ...
            logsMain.log_error(f"Autenticación fallida en { data[0] }: Más detalles: { e }")
        except:
            logsMain.log_error(f"No es posible la conexión con { data[0] }. Más detalles:")
        else:
            logsMain.log_info(f"Intentando conectar con { data[0] } ... ")
        ...
        # or absolute "C:\Users\sdkca\Desktop\TUTORIAL.txt"
            ...
            try:
                ...
                logsMain.log_info(f"Respaldo del archivo conf en el dispositivo { data[0] } realizado exitosamente!")


            except:
                logsMain.log_error(f"Error en la conexión con el dispositivo { data[0] }. ¿Está habilitado SFTP en este equipo?")
            

            # Comprobación interna para determinar si los archivos anterior y actual son los mismos o no.
            diffBackup(localDirPath + '/*', '*.txt')
