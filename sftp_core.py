...
for data in device_data:
    ...
    if os.path.exists(localFilePath):
        ...
        try:
            ...
            wlog(f"{ datetime.now().strftime('%Y-%m-%d %H:%M:%S') }   No es posible la conexión con { data[1] }: ¡tiempo agotado!")
        else:
            wlog(f"{ datetime.now().strftime('%Y-%m-%d %H:%M:%S') }   Intentando conectar { data[1] } ... ")
